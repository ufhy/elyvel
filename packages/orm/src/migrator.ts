import { existsSync } from 'node:fs'
import { join } from 'node:path'
import { sql } from 'drizzle-orm'
import type { Connection } from './connection'

/**
 * A migration file default-exports one of these. `up` applies the change;
 * `down` reverses it. Both receive the {@link Connection}; use
 * `db.execute(sql\`...\`)` for raw, portable DDL — imperative, Laravel-style.
 */
export interface Migration {
  up(db: Connection): void | Promise<void>
  down?(db: Connection): void | Promise<void>
}

const TABLE = '_ravel_migrations'

async function ensureLedger(conn: Connection): Promise<void> {
  await conn.execute(
    sql.raw(
      `CREATE TABLE IF NOT EXISTS ${TABLE} (name TEXT PRIMARY KEY, batch INTEGER NOT NULL, ran_at TEXT NOT NULL)`,
    ),
  )
}

async function alreadyRan(conn: Connection): Promise<Set<string>> {
  const rows = await conn.all<{ name: string }>(sql.raw(`SELECT name FROM ${TABLE}`))
  return new Set(rows.map((r) => r.name))
}

async function nextBatch(conn: Connection): Promise<number> {
  const rows = await conn.all<{ b: number | string }>(
    sql.raw(`SELECT COALESCE(MAX(batch), 0) AS b FROM ${TABLE}`),
  )
  return Number(rows[0]?.b ?? 0) + 1
}

interface LoadedMigration {
  name: string
  migration: Migration
}

/** Discover migration files under `dir`, sorted by filename (timestamp-first). */
export async function loadMigrations(dir: string): Promise<LoadedMigration[]> {
  if (!existsSync(dir)) return []

  const glob = new Bun.Glob('*.{ts,js}')
  const names: string[] = []
  for await (const file of glob.scan({ cwd: dir, onlyFiles: true })) {
    if (!file.endsWith('.d.ts')) names.push(file)
  }
  names.sort()

  const loaded: LoadedMigration[] = []
  for (const file of names) {
    const module = (await import(join(dir, file))) as { default?: Migration }
    if (module.default?.up) {
      loaded.push({ name: file.replace(/\.(ts|js)$/, ''), migration: module.default })
    }
  }
  return loaded
}

/** Run all pending migrations. Returns the names applied. */
export async function migrate(conn: Connection, dir: string): Promise<string[]> {
  await ensureLedger(conn)
  const ran = await alreadyRan(conn)
  const batch = await nextBatch(conn)
  const pending = (await loadMigrations(dir)).filter((m) => !ran.has(m.name))

  const applied: string[] = []
  for (const { name, migration } of pending) {
    await migration.up(conn)
    await conn.execute(
      sql`INSERT INTO ${sql.raw(TABLE)} (name, batch, ran_at) VALUES (${name}, ${batch}, ${new Date().toISOString()})`,
    )
    applied.push(name)
  }
  return applied
}

/** List user tables for the connection's dialect. */
async function userTables(conn: Connection): Promise<string[]> {
  if (conn.dialect === 'sqlite') {
    const rows = await conn.all<{ name: string }>(
      sql.raw(`SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'`),
    )
    return rows.map((r) => r.name)
  }
  const rows = await conn.all<{ name: string }>(
    sql.raw(`SELECT tablename AS name FROM pg_tables WHERE schemaname = 'public'`),
  )
  return rows.map((r) => r.name)
}

/** Drop every user table, then re-run all migrations from scratch. */
export async function freshMigrate(conn: Connection, dir: string): Promise<string[]> {
  const tables = await userTables(conn)
  const cascade = conn.dialect === 'pg' ? ' CASCADE' : ''

  if (conn.dialect === 'sqlite') await conn.execute(sql.raw('PRAGMA foreign_keys = OFF;'))
  for (const name of tables) {
    await conn.execute(sql.raw(`DROP TABLE IF EXISTS "${name}"${cascade}`))
  }
  if (conn.dialect === 'sqlite') await conn.execute(sql.raw('PRAGMA foreign_keys = ON;'))

  return migrate(conn, dir)
}
