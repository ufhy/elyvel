 ...
 */
async function post(path: string, body: Record<string, unknown>): Promise<{ error?: string, errors?: Record<string, string[]>, data?: unknown }> {
  ...
  if (!res.ok) {
    ...
    // `errors` is the framework's Laravel-shaped bag (field → messages); `error`
    // is the single summary message for a form-level alert. Both are translated.
    return { error: payload.message ?? `Request failed (${res.status})`, errors: payload.errors }
  }
  return { data }
}
